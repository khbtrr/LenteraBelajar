...
export function StudentAttendanceClient({
  ...
  const handleCheckIn = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.success) {
        setSuccessMessage(t('checkInSuccess'));
        setTokenInput('');
        router.refresh();
      } else {
        setErrorMessage(res.error || t('checkInFailed'));
      }
    } catch (err: any) {
              ...
              <div className="space-y-2 flex-1">
                <div className="flex items-center gap-2">
                  ...
                  {activeSession.moduleTitle && (
                    <Badge variant="outline" className="text-xs">
                      {t('moduleBadge', { title: activeSession.moduleTitle })}
                    </Badge>
                  )}
                </div>
                ...
                <div className="flex flex-wrap items-center gap-4 text-xs text-gray-600">
                  <span className="flex items-center gap-1">
                    ...
                    {new Date(activeSession.date).toLocaleDateString(dateLocale, {
                      weekday: 'long',
                      day: 'numeric',
                      month: 'long',
                      year: 'numeric',
                  ...
                  {activeSession.startTime && activeSession.endTime && (
                    <span className="flex items-center gap-1">
                      ...
                      {new Date(activeSession.startTime).toLocaleTimeString(dateLocale, {
                        hour: '2-digit',
                        minute: '2-digit',
                      })}{' '}
                      -{' '}
                      {new Date(activeSession.endTime).toLocaleTimeString(dateLocale, {
                        hour: '2-digit',
                        minute: '2-digit',
                      })}
                    </span>
                ...
                {isAlreadyPresent ? (
                  <div className="p-4 bg-emerald-100/70 border border-emerald-300 rounded-xl text-center space-y-1.5">
                    ...
                    <h4 className="font-bold text-emerald-900 text-sm">
                      {t('alreadyPresentTitle')}
                    </h4>
                    <p className="text-xs text-emerald-700">
                      {t('checkInTime', {
                        time: activeRecord?.checkInAt
                          ? new Date(activeRecord.checkInAt).toLocaleTimeString(dateLocale, {
                              hour: '2-digit',
                              minute: '2-digit',
                              second: '2-digit',
                            })
                  ...
                  <form onSubmit={handleCheckIn} className="space-y-3 bg-white p-4 rounded-xl border border-gray-200 shadow-sm">
                    <div className="space-y-1">
                      <Label htmlFor="token" className="text-xs font-semibold text-gray-700 flex items-center gap-1">
                        <KeyRound className="h-3.5 w-3.5 text-[#FF8928]" /> {t('enterTokenLabel')}
                      </Label>
                      <Input
                        id="token"
                    ...
                    >
                      {checkingIn ? t('processing') : t('submitCheckInBtn')}
                    </Button>
                  </form>
                )}
              </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
        <Card className="bg-white border-gray-200 shadow-sm col-span-2 sm:col-span-1">
          <CardContent className="p-4 text-center">
            <p className="text-xs font-semibold text-gray-500 uppercase">{t('attendanceRate')}</p>
            <h3 className="text-3xl font-extrabold text-[#002446] mt-1">
              {overview.attendanceRate}%
            </h3>
            <p className="text-[11px] text-gray-400 mt-0.5">{t('fromSessions', { count: overview.totalSessions })}</p>
          </CardContent>
        </Card>

        <Card className="bg-white border-gray-200 shadow-sm">
          <CardContent className="p-4 text-center">
            <p className="text-xs font-semibold text-emerald-700 uppercase">{t('present')}</p>
            <h3 className="text-2xl font-bold text-emerald-600 mt-1">{overview.presentCount}</h3>
            <p className="text-[11px] text-gray-400 mt-0.5">{t('sessionsUnit')}</p>
          </CardContent>
        </Card>

        <Card className="bg-white border-gray-200 shadow-sm">
          <CardContent className="p-4 text-center">
            <p className="text-xs font-semibold text-amber-700 uppercase">{t('sick')}</p>
        ...
        <Card className="bg-white border-gray-200 shadow-sm">
          <CardContent className="p-4 text-center">
            <p className="text-xs font-semibold text-blue-700 uppercase">{t('permission')}</p>
        ...
        <Card className="bg-white border-gray-200 shadow-sm">
          <CardContent className="p-4 text-center">
            <p className="text-xs font-semibold text-rose-700 uppercase">{t('absent')}</p>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left text-sm">
                <thead className="bg-gray-50 border-y border-gray-200 text-xs font-semibold text-gray-600 uppercase">
                  <tr>
                    <th className="px-4 py-3">{t('colNo')}</th>
                    <th className="px-4 py-3">{t('colSession')}</th>
                    <th className="px-4 py-3">{t('colDate')}</th>
                    <th className="px-4 py-3">{t('colCheckInTime')}</th>
                    ...
                    return (
                      <tr key={h.sessionId} className="hover:bg-gray-50/70 transition-colors">
                        ...
                        <td className="px-4 py-3">
                          ...
                          {h.moduleTitle && (
                            <div className="text-xs text-gray-500">{t('moduleBadge', { title: h.moduleTitle })}</div>
                          )}
                        </td>
                        ...
                        <td className="px-4 py-3 text-xs text-gray-500 font-mono">
                          {h.checkInAt
                            ? new Date(h.checkInAt).toLocaleTimeString(dateLocale, {
                                hour: '2-digit',
                                minute: '2-digit',
                              })
                            : '-'}
