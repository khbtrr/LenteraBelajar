...
export default async function AdminDashboard() {
  ...
  const [userCount, courseCount, studentCount, pendingRequests] = await Promise.all([
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            <CardTitle className="text-sm font-medium text-gray-600">{t('pendingRequests')}</CardTitle>
            <ClipboardCheck className="h-5 w-5 text-[#FF8928]" />
          </CardHeader>
          <CardContent>
            <p className="text-3xl font-bold text-[#002446]">{pendingRequests}</p>
          </CardContent>
        </Card>
      </div>
    </div>
